...
const formatearFila = async (data, folio) => {
	...
	for (let index = 1; index < sortedData.length; index++) {
		...
		const ficha = Object.keys(columnIndexes).reduce((ficha, columnName) => {
			...
			if (['id', 'latitud', 'longitud', 'superficie_terreno', 'superficie_construccion', 'valor_terreno', 'valor_construccion', 'valor_catastral', 'total_pagado', 'descuento'].includes(columnName) && ficha[columnName] === 'desconocido') {
				ficha[columnName] = 0
			}
			return ficha
		}, {})
