...
export default function Data({ data, setData, seleccion }) {
    ...
    return (
        ...
        <Box sx={{ position: 'absolute', display: data ? 'flex' : 'none', justifyContent: 'center', alignItems: 'center', width: '100%', height: '100vh', background: 'rgba(0,0,0,0.5)', top: '0', right: '0', zIndex: '9999' }}>
            
			<Box sx={{ width: '95%', maxWidth: '1000px', height:'auto', maxHeight:'600px', minHeight:'600px', borderRadius: '10px', backgroundColor: '#17212F', border: '2px solid #fff', padding: '20px', overflowX: 'hidden', overflowY: 'scroll' }}>
                
				<Box sx={{ width: '100%', display: 'flex', justifyContent: 'space-between' }}>
                    <Typography sx={{ color: '#fff', fontSize: '30px', fontWeight: '500' }}>Información del registro</Typography>
                    <Button onClick={() => setData(false)}>
                        <CloseFullscreenIcon sx={{ color: 'red', fontSize: '30px', fontWeight: '500' }} />
                    </Button>
                </Box>
					...
					<Box sx={{ width:'100%', display:openTerrenoData ? 'flex' : 'none', justifyContent:'center', alignItems:'center', flexDirection:'column', mb:'0px' }}>
						...
						<TableContainer component={Paper}>
							...
							<Table sx={{ minWidth: 650 }} aria-label="simple table">
								...
								<TableBody>
									{seleccion && seleccion.catastroTerreno ? (
										seleccion.catastroTerreno.map((terreno) => (
					...
					<Box sx={{ width:'100%', display:openConstruccionData ? 'flex' : 'none', justifyContent:'center', alignItems:'center', flexDirection:'column', mb:'20px' }}>
						...
						<TableContainer component={Paper}>
							...
							<Table sx={{ minWidth: 650 }} aria-label="simple table">
								...
								<TableBody>
									{seleccion && seleccion.catastroConstruccion ? (
										seleccion.catastroConstruccion.map((constuccion, index) => (
					...
					<Box sx={{ width:'100%', display:openAdeudoData ? 'flex' : 'none', justifyContent:'center', alignItems:'center', flexDirection:'column', mb:'20px' }}>
						...
						<TableContainer component={Paper}>
							...
							<Table sx={{ minWidth: 650 }} aria-label="simple table">
								...
								<TableBody>
									{seleccion && seleccion.catastroAdeudo ? (
										seleccion.catastroAdeudo.map((constuccion, index) => (
										<TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
...
Data.propTypes = {	
    ...
    seleccion: PropTypes.shape({
		...
		catastroTerreno: PropTypes.any,
		catastroConstruccion: PropTypes.any,
		catastroAdeudo: PropTypes.any
    })
}