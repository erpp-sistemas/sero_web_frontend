	...
	const Impression = () => {
		...
		const handleStartUp = () => {
			...
			const start = Math.max(0, rangoInicial - 1)
			const end = Math.min(rangoFinal, registrosArray.length)
		
			for (let i = start; i < end; i++) {
				handleGeneratePDF(registrosArray[i])
			}
		}
		...
		useEffect(() => {
			if (completedRequests === rangoFinal) {
				setCompletedRequests(0)
				setLoading(false)
			}
		}, [completedRequests, rangoFinal])
