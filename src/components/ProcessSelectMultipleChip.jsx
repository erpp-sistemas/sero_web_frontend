...
function ProcessSelectMultipleChip({
  ...
  const safeSelectedProcess = Array.isArray(selectedProcess)
    ? selectedProcess
    : [];
    ...
    const loadProcesses = async () => {
      if (selectedPlace && selectedService) {
        ...
        try {
          ...
          const safeProcesses = Array.isArray(res) ? res : [];
          setProcess(safeProcesses);

          if (safeSelectedProcess.length === 0 && safeProcesses.length > 0) {
            handleProcessChange({
              target: { value: [safeProcesses[0].process_id] },
            });
          }
        } catch (error) {
  ...
  const selectedProcessNames = safeSelectedProcess.map(
    (selectedId) =>
