...
const GeneralDataModal = ({ open, onClose, data, resetTrigger }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "name") {
      if (
        value !== nameOriginal ||
        formData.first_last_name !== firstLastNameOriginal
      ) {
        setGenerateNew(true);
      } else {
  ...
  const getModifiedData = () => {
    ...
    const modifiedData = Object.keys(formData).reduce((acc, key) => {
      if (formData[key] !== originalData[key]) {
        if (key === "user_name" && formData[key] === generatedValues.user_name)
          return acc;
  ...
  const handleSubmit = async (e) => {
    ...
    const hasChanges = Object.values(modifiedData).some(
      (value) => value !== ""
    );
                        ...
                        InputProps={{
                          ...
                          sx: {
                            '& input[type="date"]::-webkit-calendar-picker-indicator':
                              {
                                filter:
                                  "invert(50%) sepia(100%) saturate(1000%) hue-rotate(90deg) brightness(1.2) contrast(1.2)",
                              },
                  ...
                  <Grid container mt={1} spacing={2} justifyContent="center">
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <CloudUploadIcon sx={{ color: colors.accentGreen[100]}} fontSize="large" />
                          <Avatar
                            alt="Foto"
                            src={
                              formData.url_image
