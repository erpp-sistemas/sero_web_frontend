...
function PaymentsProceduresByAmountPaid({ data, typeConcept }) {
  ...
  const groupedData = data.reduce((acc, curr) => {
    const { month_year, concept, amount_paid } = curr;
    if (!acc[month_year]) {
      acc[month_year] = {
        id: month_year,
        color: generateColor(),
    ...
    acc[month_year].data.push({ x: concept.toString(), y: amount_paid });
    return acc;
  }, {});
      ...
      >
            ...
            >
                ...
                <Typography
                    ...
                    textAlign={'center'}
                >
                    MONTO PAGADO POR GESTIONES REALIZADAS
                </Typography>
            </Box>
            {data.length > 0 && (
                <Line data={ formattedData } titlex={'dias del mes'} />
              )}
        </Box>
    </Box>
  )
...
export default PaymentsProceduresByAmountPaid